                ...
                try:
                    ...
                    if masked_pronoun:
                        ...
                        for j, tok in enumerate(pred_tokens):
                            preds[str(j + 1)] = (tok, round(float(top_probs[j] * 100), 4), float(pred_weights[j]))
...
if __name__ == "__main__":
    ...
    mkdir(log_dir)
    logging.basicConfig(filename=pjoin(log_dir, "maksed_token_"+"-".join(re.split(":|\.| ", str(datetime.datetime.now()))[:-1])+".log"),
                        level=logging.INFO)
